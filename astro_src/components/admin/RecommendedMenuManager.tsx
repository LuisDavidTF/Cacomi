...
export function RecommendedMenuManager() {
    ...
    const generateCode = () => {
        ...
        days.forEach(day => {
            grouped[day] = selection
                .filter(m => m.dayOfWeek === day)
                .map(m => {
                    const { dayOfWeek, ...cleanMeal } = m;
                    return cleanMeal;
                });
        });
        const code = `export const RECOMMENDED_WEEKLY_MENU: Record<string, Omit<LocalPlannedMeal, 'id' | 'planId' | 'mealDate' | 'isSynced' | 'isDeleted' | 'isNew'>[]> = ${JSON.stringify(grouped, null, 4)};`;
        return code;
    };
                            ...
                            {selection.length === 0 ? (
                                ...
                                <div className="space-y-8">
                                    {['lun', 'mar', 'mie', 'jue', 'vie', 'sab', 'dom'].map((day) => {
                                        const dayMeals = selection.filter(m => m.dayOfWeek === day);
                                        if (dayMeals.length === 0) return null;

                                        return (
                                            <div key={day} className="space-y-4">
                                                <h3 className="text-xs font-black uppercase tracking-widest text-primary border-b border-border pb-2">
                                                    {day.toUpperCase()} ({dayMeals.length} comidas)
                                                </h3>
                                                
                                                <div className="space-y-4">
                                                    {dayMeals.map((meal) => (
                                                        <div key={`${day}-${meal.recipeUUID}`} className="p-4 sm:p-5 bg-muted/30 rounded-[1.5rem] sm:rounded-[2rem] border border-border/50 group space-y-4">
                                                            <div className="flex items-start gap-3 sm:gap-4">
                                                                <img src={meal.imageUrl} className="w-16 h-16 sm:w-20 sm:h-20 rounded-xl sm:rounded-2xl object-cover shadow-lg shrink-0" />
                                                                <div className="flex-1 min-w-0">
                                                                    ...
                                                                    <div className="flex flex-wrap gap-1 mb-2.5">
                                                                        {(['BREAKFAST', 'SNACK_1', 'LUNCH', 'SNACK_2', 'DINNER'] as const).map((type) => (
                                                                            <button
                                                                                key={type}
                                                                            ...
                                                                            >
                                                                                {type === 'BREAKFAST' ? 'Desayuno' : 
                                                                                 type === 'SNACK_1' ? 'Colación 1' : 
                                                                                 type === 'LUNCH' ? 'Almuerzo' : 
                                                                                 type === 'SNACK_2' ? 'Colación 2' : 'Cena'}
                                                                            </button>
                                                                        ))}
                                                                    </div>
                                                                    ...
                                                                    <div className="flex flex-wrap gap-1">
                                                                        {['lun', 'mar', 'mie', 'jue', 'vie', 'sab', 'dom'].map((d) => (
                                                                            <button
                                                                                key={d}
                                                                                onClick={() => updateMealField(meal.recipeUUID, 'dayOfWeek', d as any)}
                                                                                className={cn(
                                                                                    "px-2 py-0.5 rounded-lg text-[7px] sm:text-[8px] font-black uppercase tracking-widest border transition-all",
                                                                                    meal.dayOfWeek === d
                                                                                        ? "bg-amber-500 text-white border-amber-500 shadow-lg shadow-amber-500/20" 
                                                                                        : "bg-white dark:bg-gray-800 border-border hover:border-amber-500/30"
                                                                                )}
                                                                            >
                                                                                {d.toUpperCase()}
                                                                            </button>
                                ...
                                {(['BREAKFAST', 'SNACK_1', 'LUNCH', 'SNACK_2', 'DINNER'] as const).map(type => {
                                    ...
                                    return (
                                        <div key={type} className={cn(
                                            ...
                                            count === 0 ? "bg-muted text-muted-foreground border-transparent" : "bg-green-500/10 text-green-600 border-green-500/20"
                                        )}>

...
export function MealTrackingModal({ isOpen, onClose, mealData, onSave }: MealTrackingModalProps) {
    ...
    return (
        ...
        >
            <div className={`bg-background border border-border/50 shadow-2xl rounded-3xl w-full flex flex-col lg:flex-row overflow-hidden transition-all duration-500 ease-in-out h-[85vh] sm:h-[800px] ${isViewingRecipe ? 'max-w-4xl' : 'max-w-xl'}`}>
                
                {/* Left Side: Tracking Form */}
                <div className={`flex flex-col w-full h-full overflow-hidden transition-all duration-500 ${isViewingRecipe ? 'hidden lg:flex lg:w-[420px] shrink-0 border-r border-border/30' : 'flex-1'}`}>
                ...
                <div className="relative h-[380px] sm:h-[480px] shrink-0 bg-muted/20 group overflow-hidden">
                    ...
                    <div className="absolute top-5 left-5 flex flex-col gap-2">
                        <span className="w-fit px-3 py-1.5 rounded-xl bg-primary text-primary-foreground text-[10px] font-black uppercase tracking-[0.2em] shadow-xl shadow-primary/40 backdrop-blur-md border border-white/10">
                            {mealData.mealType}
                        </span>
                        {mult !== 1.0 && (
                            <span className="w-fit px-3 py-1.5 rounded-xl bg-amber-500 text-white text-[10px] font-black uppercase tracking-[0.2em] shadow-xl shadow-amber-500/40 backdrop-blur-md border border-white/10">
                                {mult}x PORCIÓN
                            </span>
                        )}
                    </div>
                    ...
                    <button 
                        onClick={onClose} 
                        className="absolute top-5 right-5 p-3 bg-black/20 backdrop-blur-xl text-white hover:bg-black/40 rounded-2xl transition-all duration-300 hover:rotate-90"
                    >
                        <X className="w-5 h-5" />
                    </button>
                    
                    <div className="absolute bottom-0 left-0 right-0 p-8 flex flex-col justify-end transition-all duration-500">
                        <h2 className={`font-black text-white leading-[1.1] mb-6 drop-shadow-2xl tracking-tighter transition-all duration-500 ${isViewingRecipe ? 'text-2xl sm:text-3xl' : 'text-4xl sm:text-5xl'}`}>
                            {mealData.recipeName}
                        </h2>
                        
                        <div className={`flex items-center gap-4 sm:gap-5 mb-8 py-4 px-5 bg-black/20 backdrop-blur-2xl border border-white/10 rounded-3xl shadow-2xl transition-all duration-500 ${isViewingRecipe ? 'scale-90 origin-left' : ''}`}>
                            <div className="flex flex-col">
                                <span className="text-[10px] uppercase font-black text-white/50 tracking-widest mb-1">
                                    {trackingTexts?.stats?.energy || 'Energía'}
                                </span>
                                <div className="flex items-center gap-2 text-white font-black text-base">
                                    <Flame className="w-4 h-4 text-orange-400" />
                                    {mealData.calories?.toFixed(0)} <span className="text-[10px] font-medium opacity-60">KCAL</span>
                                </div>
                            </div>
                            ...
                            <div className="flex flex-col">
                                ...
                                <div className="flex items-center gap-2 text-white font-black text-base">
                                    ...
                                    {mealData.proteinGrams?.toFixed(0)}<span className="text-[10px] font-medium opacity-60">G</span>
                                </div>
                            </div>
                            ...
                            <div className="flex flex-col">
                                ...
                                <div className="flex items-center gap-2 text-white font-black text-base">
                                    ...
                                    <span className="text-[10px] font-medium opacity-60">$</span>{mealData.estimatedCost?.toFixed(2)}
                                </div>
                            </div>
                        </div>
                ...
                {/* Body: Tracking Form */}
                <div className="p-6 md:p-8 overflow-y-auto overscroll-contain flex-1 custom-scrollbar min-h-0 bg-background">
                    {mult !== 1.0 && (
                ...
                {/* Footer */}
                <div className="p-4 border-t border-border/30 bg-muted/10 shrink-0 mt-auto">
                    <button
                        onClick={handleSave}
